...
if(isset($_POST['form7_1'])) {
    ...
    if($path == '') {
        ...
        $ext = pathinfo( $path, PATHINFO_EXTENSION );
        $file_name = basename( $path, '.' . $ext );
        if( $ext!='jpg' && $ext!='JPG' && $ext!='png' && $ext!='PNG' && $ext!='jpeg' && $ext!='JPEG' && $ext!='gif' && $ext!='PNG' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file<br>';
        }
    }
...
if(isset($_POST['form10'])) {
    // updating the database
    $statement = $pdo->prepare("UPDATE tbl_settings SET before_head=?, after_body=?, before_body=?, before_body_next=? WHERE id=1");
    $statement->execute(array($_POST['before_head'],$_POST['after_body'],$_POST['before_body'], $_POST['before_body_next']));
...
foreach ($result as $row) {
    ...
    $before_body_next                = $row['before_body_next'];
                                    ...
                                    <div class="box-body">
                                        ...
                                        <div class="form-group">
                                          ...
                                          <div class="form-group">
                                            <label for="" class="col-sm-2 control-label">Code before &lt;/body&gt; tag </label>
                                            <div class="col-sm-8">
                                                <textarea name="before_body_next" class="form-control" cols="30" rows="10"><?php echo $before_body_next; ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="col-sm-2 control-label"></label>
